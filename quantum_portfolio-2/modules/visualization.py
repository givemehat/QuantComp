...
def efficient_frontier_chart(
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers+text",
            marker=dict(
                symbol="star", size=22, color=_AMBER, line=dict(color=_TEXT, width=1.5)
            ),
            text=["Max Sharpe"],
...
def bitstring_probability_chart(
    ...
    for bits, prob in sorted_items:
        ...
        vals.append(prob)
        colors.append(
            _AMBER if bits == best_raw_bitstring else _hex_to_rgba(_INDIGO, 0.75)
        )
    ...
    layout = _base_layout(
        "QAOA Measurement Distribution  (Top Solutions)", height=max(340, top_k * 34)
    )
...
def cumulative_returns_chart(
    ...
    fig.add_hline(
        ...
        annotation=dict(
            text="Initial $1",
            font=dict(color=_MUTED, size=9, family=FONT_MONO),
            xref="paper",
            x=0.01,
        ),
    )
