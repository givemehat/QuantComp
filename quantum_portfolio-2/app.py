...
with st.sidebar:
    ...
    tickers = list(
        dict.fromkeys(tickers_preset + custom_tickers)
    ...
    use_cache = st.checkbox(
        "Use disk cache", value=True, help="Uncheck to force a fresh download."
    )
    if cache_info:
        st.caption(
...
if not run and "qpl_results" not in st.session_state:
    ...
    info(
        "Configure the sidebar and click <strong>Run Analysis</strong> to start the pipeline."
    )
    st.stop()
...
if len(tickers) > 8:
    warn(
        "More than 8 tickers increases QAOA circuit complexity significantly. "
...
if run:
    ...
    with st.spinner("Downloading market data..."):
        try:
            ...
            n_stocks = len(tickers)
            expected_returns_series, cov_matrix, log_returns = compute_annualised_stats(
                prices
            )
    ...
    with st.spinner("Training forecasting model..."):
        try:
            if use_lstm:
                ...
                X_tr, y_tr, X_val, y_val = prepare_lstm_sequences(
                    scaled, n_steps=n_steps
                )
                if len(X_tr) < 32:
                    warn(
                        "Training set is very small (<32 samples). "
                        "Try a longer period or a smaller look-back window."
                    )
                lstm = build_model(input_shape=(n_steps, n_stocks), n_outputs=n_stocks)
                history = train(
                    lstm, X_tr, y_tr, X_val, y_val, epochs=epochs, patience=patience
                )
                if wf_mode:
    ...
    with st.spinner("Running classical optimization..."):
        try:
            ...
            ms_ret, ms_vol, ms_sharpe = portfolio_stats(
                ms_weights, mu_series, cov_matrix, risk_free
            )
            ...
            mv_ret, mv_vol, mv_sharpe = portfolio_stats(
                mv_weights, mu_series, cov_matrix, risk_free
            )
            ...
            fvols, frets = efficient_frontier(
                mu_series, cov_matrix, risk_free, n_points=60, max_weight=max_weight
            )
    ...
    with st.spinner(f"Running QAOA (p={reps}, {optimizer_name})… 30–120 s"):
        try:
            ...
            q_var_m = (
                value_at_risk(q_weights, log_returns) if q_weights.sum() > 0 else 0.0
            )
            q_cvar_m = (
                conditional_var(q_weights, log_returns) if q_weights.sum() > 0 else 0.0
            )
            q_sortino = (
                sortino_ratio(q_weights, log_returns, risk_free)
                if q_weights.sum() > 0
                else 0.0
            )
            q_calmar = (
                calmar_ratio(q_weights, log_returns, prices, risk_free)
                if q_weights.sum() > 0
                else 0.0
            )
            q_mdd = (
                max_drawdown_from_weights(q_weights, prices)
                if q_weights.sum() > 0
                else 0.0
            )

            # Brute-force reference (feasible n ≤ 8)
            bf_sel, bf_score = brute_force_optimal(
                predicted_returns, cov_matrix, risk_aversion
            )
            ...
            st.session_state["qpl_results"] = {
                ...
                "rolling_sharpe_df": rolling_sharpe_df,
                "cov_matrix": cov_matrix,
                "mu_series": mu_series,
                "predicted_returns": predicted_returns,
                "history": history,
...
# ══════════════════════════════════════════════════════════════════════════════
selected_display = (
    " · ".join(qaoa_result.selected_tickers) if qaoa_result.selected_tickers else "None"
)
st.markdown(
    f"""
...
with m1:
    st.metric(
        "QAOA Return", f"{q_ret*100:+.2f}%", delta=f"vs MS {(q_ret-ms_ret)*100:+.2f}%"
    )
with m2:
    st.metric("QAOA Vol", f"{q_vol*100:.2f}%")
with m3:
    st.metric(
        "QAOA Sharpe", f"{q_sharpe:.3f}", delta=f"vs MS {(q_sharpe-ms_sharpe):+.3f}"
    )
with m4:
    st.metric("Max Sharpe", f"{ms_sharpe:.3f}")
with m5:
    st.metric("QAOA VaR 95%", f"{q_var_m*100:.2f}%")
with m6:
    st.metric(
        "Approx Ratio",
        f"{approx_ratio_val:.3f}",
...
with tab_data:
    ...
    with c2:
        ...
        st.plotly_chart(
            rolling_sharpe_chart(rolling_sharpe_df), use_container_width=True
        )
    ...
    ret_df = pd.DataFrame(
        {
            ...
            "Selected (QAOA)": [
                "Yes" if t in qaoa_result.selected_tickers else "No" for t in tickers
            ],
        }
    ).set_index("Ticker")
...
with tab_risk:
    ...
    with c1:
        ...
        st.plotly_chart(
            covariance_heatmap(cov_matrix, tickers), use_container_width=True
        )
    with c2:
    ...
    def norm_vals(vals):
        return [_norm(v, min(vals), max(vals)) for v in vals]

    ms_radar = [
        norm_vals(all_rets)[0],
        1 - norm_vals(all_vols)[0],
        norm_vals(all_sharpes)[0],
        1 - norm_vals(all_var)[0],
        1 - norm_vals(all_cvar)[0],
    ]
    mv_radar = [
        norm_vals(all_rets)[1],
        1 - norm_vals(all_vols)[1],
        norm_vals(all_sharpes)[1],
        1 - norm_vals(all_var)[1],
        1 - norm_vals(all_cvar)[1],
    ]
    q_radar = [
        norm_vals(all_rets)[2],
        1 - norm_vals(all_vols)[2],
        norm_vals(all_sharpes)[2],
        1 - norm_vals(all_var)[2],
        1 - norm_vals(all_cvar)[2],
    ]
    st.plotly_chart(
        risk_radar(radar_labels, ms_radar, mv_radar, q_radar), use_container_width=True
    )
...
with tab_quantum:
    ...
    with c1:
        st.metric("Circuit depth p", reps)
    with c2:
        st.metric("Qubits", qaoa_result.n_qubits)
    with c3:
        st.metric("Optimizer", optimizer_name)
    with c4:
        st.metric("Risk aversion λ", risk_aversion)
    ...
    with cols_bf[0]:
        st.metric("Brute-force Optimal", " · ".join(bf_tickers_sel))
    with cols_bf[1]:
        st.metric("QAOA Score", f"{q_score:.6f}")
    with cols_bf[2]:
        st.metric("Optimal Score", f"{bf_score:.6f}")
